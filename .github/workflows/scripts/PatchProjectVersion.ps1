...
function Update-SourceVersion
{
  ...
  $NewVersion = '<AssemblyVersion>' + $Version + '</AssemblyVersion>';
  $NewFileVersion = '<FileVersion>' + $Version + '</FileVersion>';
  ...
  foreach ($o in $input) 
  {
    ...
    $TmpFile = $o.FullName + ".tmp"
     ...
     Get-Content $o.FullName -encoding utf8 |
        ...
        %{$_ -replace '<FileVersion>[0-9]+(\.([0-9]+|\*)){1,3}</FileVersion>', $NewFileVersion } |
...
{
  get-childitem -Path *.csproj -recurse | Update-SourceVersion $version; 
}
